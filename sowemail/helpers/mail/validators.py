...
class ValidateApiKey(object):
    """Validates content to ensure SoWeMail API key is not present"""
    ...
    def validate_message_dict(self, request_body):
        """With the JSON dict that will be sent to SoWeMail's API,
            check the content for SoWeMail API keys - throw exception if found.

           :param request_body: The JSON dict that will be sent to SoWeMail's
                                API.
    ...
    def validate_message_text(self, message_string):
        """With a message string, check to see if it contains a SoWeMail API Key
